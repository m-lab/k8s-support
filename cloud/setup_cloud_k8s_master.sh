...
REGION=${GOOGLE_CLOUD_REGION:-us-central}
ZONE=${GOOGLE_CLOUD_REGION:-us-central1-c}
...
gcloud compute instances create $GCE_NAME \
  --image "ubuntu-1604-xenial-v20180323" --image-project "ubuntu-os-cloud" --boot-disk-size "10" --boot-disk-type "pd-standard" --boot-disk-device-name $GCE_NAME  \
  --tags "dmz" \
...
#  a bit.
until gcloud compute ssh $GCE_NAME --command true && gcloud compute ssh $GCE_NAME --command true; do
...
docker run -i \
  ...
  xakra/kubeadm-installer:0.4.5 ubuntu install || true
EOF
